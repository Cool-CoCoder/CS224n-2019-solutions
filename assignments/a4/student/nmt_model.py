...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # 注意这里decoder的size
        self.decoder = nn.LSTM(hidden_size+embed_size,hidden_size,bias=True)
        self.h_projection = nn.Linear(2*hidden_size,hidden_size,bias=False)
        self.c_projection = nn.Linear(hidden_size,2*hidden_size,bias=False)
        self.att_projection = nn.Linear(2*hidden_size,hidden_size,bias=False)
        self.combined_output_projection = nn.Linear(3*hidden_size,hidden_size,bias=False)
        # object of type 'Vocab' has no len()
        #RuntimeError: mat1 and mat2 shapes cannot be multiplied (1344x768 and 8001x2304)
        # RuntimeError: mat1 and mat2 shapes cannot be multiplied (1344x768 and 2304x8001)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # nn.utils.rnn.pack_padded_sequence和nn.utils.rnn.pack_padded_sequence 两者是互逆
        #  Module [ModelEmbeddings] is missing the required "forward" function
        ...
        # RuntimeError: mat1 and mat2 shapes cannot be multiplied (5x8 and 4x2)

        init_decoder_hidden = self.h_projection(torch.concat((last_hidden[0],last_hidden[1]),dim=1))
        init_decoder_cell = self.h_projection(torch.concat((last_cell[0],last_cell[1]),dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###             - Use the step function(这里的step function是什么? 就是下面的函数step) step to compute the the Decoder's next (cell, state) values
        ...
        for Y_t in torch.split(Y,1,dim=0):
            ...
            #  expected Tensor as element 1 in argument 0, but got NoneType
            # print('Y_t,o_prev:',Y_t.shape,o_prev.shape)
            Ybar_t = torch.concat((Y_t,o_prev),dim=-1)
            # 在step没有完成之前,这个o_t会是None,导致循环后续执行不了,那这个sanity_check的意义在哪呢?
            output = self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
            dec_state, o_t, e_t = output
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Ybar_t shape  (b, e + h)
        output,dec_state = self.decoder(Ybar_t.unsqueeze(0),(hidden_state.unsqueeze(0), cell_state.unsqueeze(0)))
        dec_hidden, dec_cell = dec_state
        dec_state = (dec_hidden.squeeze(0),dec_cell.squeeze(0))
        # enc_hiddens_proj shape (b, src_len, h)
        # e_t is attention score
